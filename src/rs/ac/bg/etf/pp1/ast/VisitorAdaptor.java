// generated with ast extension for cup
// version 0.8
// 4/1/2023 23:30:27


package rs.ac.bg.etf.pp1.ast;

public abstract class VisitorAdaptor implements Visitor { 

    public void visit(MethodDecl MethodDecl) { }
    public void visit(Mulop Mulop) { }
    public void visit(Relop Relop) { }
    public void visit(MethodType MethodType) { }
    public void visit(NonLastVarDecl NonLastVarDecl) { }
    public void visit(StatementList StatementList) { }
    public void visit(LastVarDecl LastVarDecl) { }
    public void visit(Addop Addop) { }
    public void visit(Factor Factor) { }
    public void visit(CondTerm CondTerm) { }
    public void visit(AssignOpExpression AssignOpExpression) { }
    public void visit(MethodDeclListClass MethodDeclListClass) { }
    public void visit(FormParsBrackets FormParsBrackets) { }
    public void visit(Designator Designator) { }
    public void visit(Term Term) { }
    public void visit(FormParsList FormParsList) { }
    public void visit(Condition Condition) { }
    public void visit(ActParsList ActParsList) { }
    public void visit(IfCondition IfCondition) { }
    public void visit(ListConstDecls ListConstDecls) { }
    public void visit(ConstructorDeclList ConstructorDeclList) { }
    public void visit(ClassExtends ClassExtends) { }
    public void visit(VarDeclList VarDeclList) { }
    public void visit(Expr Expr) { }
    public void visit(ClassFunctions ClassFunctions) { }
    public void visit(ActPars ActPars) { }
    public void visit(FormParsListWithComa FormParsListWithComa) { }
    public void visit(DesignatorList DesignatorList) { }
    public void visit(DesignatorStatement DesignatorStatement) { }
    public void visit(ClassFields ClassFields) { }
    public void visit(Statement Statement) { }
    public void visit(ListVarDecls ListVarDecls) { }
    public void visit(VarBrackets VarBrackets) { }
    public void visit(CondFact CondFact) { }
    public void visit(ConstDecls ConstDecls) { }
    public void visit(MethodDeclList MethodDeclList) { }
    public void visit(EpsilonDesignator EpsilonDesignator) { }
    public void visit(NonEmptyDesignatorList NonEmptyDesignatorList) { }
    public void visit(FormPars FormPars) { }
    public void visit(Percent Percent) { visit(); }
    public void visit(Div Div) { visit(); }
    public void visit(Mul Mul) { visit(); }
    public void visit(Minus Minus) { visit(); }
    public void visit(Plus Plus) { visit(); }
    public void visit(NotMore NotMore) { visit(); }
    public void visit(Less Less) { visit(); }
    public void visit(NotLess NotLess) { visit(); }
    public void visit(More More) { visit(); }
    public void visit(NotEquals NotEquals) { visit(); }
    public void visit(Equals Equals) { visit(); }
    public void visit(Assignop Assignop) { visit(); }
    public void visit(Label Label) { visit(); }
    public void visit(ArrayDesignator ArrayDesignator) { visit(); }
    public void visit(DotDesignator DotDesignator) { visit(); }
    public void visit(IdentDesignator IdentDesignator) { visit(); }
    public void visit(FactorExpr FactorExpr) { visit(); }
    public void visit(FactorNewArray FactorNewArray) { visit(); }
    public void visit(FactorNewObj FactorNewObj) { visit(); }
    public void visit(FactorBool FactorBool) { visit(); }
    public void visit(FactorChar FactorChar) { visit(); }
    public void visit(FactorNumber FactorNumber) { visit(); }
    public void visit(FactorDesignatorActPars FactorDesignatorActPars) { visit(); }
    public void visit(FactorDesignator FactorDesignator) { visit(); }
    public void visit(MulTerm MulTerm) { visit(); }
    public void visit(NormalTerm NormalTerm) { visit(); }
    public void visit(AddopExpr AddopExpr) { visit(); }
    public void visit(PositiveExpr PositiveExpr) { visit(); }
    public void visit(NegativeExpr NegativeExpr) { visit(); }
    public void visit(ConditionWithoutOp ConditionWithoutOp) { visit(); }
    public void visit(ConditionWithOp ConditionWithOp) { visit(); }
    public void visit(JustCondTerm JustCondTerm) { visit(); }
    public void visit(ConditionAnd ConditionAnd) { visit(); }
    public void visit(JustCondition JustCondition) { visit(); }
    public void visit(ConditionDerived1 ConditionDerived1) { visit(); }
    public void visit(ConditionOR ConditionOR) { visit(); }
    public void visit(SingleActParsList SingleActParsList) { visit(); }
    public void visit(ActParsListWithComa ActParsListWithComa) { visit(); }
    public void visit(EmptyActPars EmptyActPars) { visit(); }
    public void visit(ActParsWithList ActParsWithList) { visit(); }
    public void visit(DesignatorDoesntExist DesignatorDoesntExist) { visit(); }
    public void visit(DesignatorExists DesignatorExists) { visit(); }
    public void visit(SingleDesignatorList SingleDesignatorList) { visit(); }
    public void visit(NonEmptyDesignatorListWithComa NonEmptyDesignatorListWithComa) { visit(); }
    public void visit(EmptyDesignatorList EmptyDesignatorList) { visit(); }
    public void visit(DesignatorListNonEmpty DesignatorListNonEmpty) { visit(); }
    public void visit(DesignatorFuncCall DesignatorFuncCall) { visit(); }
    public void visit(DesignatorDecrement DesignatorDecrement) { visit(); }
    public void visit(DesignatorIncrement DesignatorIncrement) { visit(); }
    public void visit(DesignatorFunc DesignatorFunc) { visit(); }
    public void visit(DesignatorStatementAssignOp DesignatorStatementAssignOp) { visit(); }
    public void visit(DesignatorArray DesignatorArray) { visit(); }
    public void visit(ErrorDesignatorAssign ErrorDesignatorAssign) { visit(); }
    public void visit(DesignatorAssign DesignatorAssign) { visit(); }
    public void visit(ErrorIfConditionStatement ErrorIfConditionStatement) { visit(); }
    public void visit(IfConditionStatement IfConditionStatement) { visit(); }
    public void visit(ForEachStart ForEachStart) { visit(); }
    public void visit(WhileStart WhileStart) { visit(); }
    public void visit(BlockStatement BlockStatement) { visit(); }
    public void visit(ForeachStatement ForeachStatement) { visit(); }
    public void visit(PrintNumberStatement PrintNumberStatement) { visit(); }
    public void visit(PrintStatement PrintStatement) { visit(); }
    public void visit(ReadStatement ReadStatement) { visit(); }
    public void visit(ReturnStatement ReturnStatement) { visit(); }
    public void visit(VoidReturnStatement VoidReturnStatement) { visit(); }
    public void visit(ContinueStatement ContinueStatement) { visit(); }
    public void visit(BreakStatement BreakStatement) { visit(); }
    public void visit(WhileStatement WhileStatement) { visit(); }
    public void visit(IfElseStatement IfElseStatement) { visit(); }
    public void visit(IfStatement IfStatement) { visit(); }
    public void visit(DesignatorStatementState DesignatorStatementState) { visit(); }
    public void visit(EmptyStatementList EmptyStatementList) { visit(); }
    public void visit(StatementListWithStatement StatementListWithStatement) { visit(); }
    public void visit(FormParsDecl FormParsDecl) { visit(); }
    public void visit(FormParList FormParList) { visit(); }
    public void visit(FormParsListWithComaList FormParsListWithComaList) { visit(); }
    public void visit(FormParsListError FormParsListError) { visit(); }
    public void visit(FormParsListWithComaDone FormParsListWithComaDone) { visit(); }
    public void visit(FormParsError FormParsError) { visit(); }
    public void visit(FormParsEmptyList FormParsEmptyList) { visit(); }
    public void visit(FormParsWithList FormParsWithList) { visit(); }
    public void visit(FormParsBracketsExists FormParsBracketsExists) { visit(); }
    public void visit(MethodWithNoType MethodWithNoType) { visit(); }
    public void visit(MethodWithType MethodWithType) { visit(); }
    public void visit(MethodDeclaration MethodDeclaration) { visit(); }
    public void visit(NoMethodDeclList NoMethodDeclList) { visit(); }
    public void visit(HasMethodDeclList HasMethodDeclList) { visit(); }
    public void visit(NoMethodDeclListClass NoMethodDeclListClass) { visit(); }
    public void visit(HasMethodDeclListClass HasMethodDeclListClass) { visit(); }
    public void visit(ConstructorDecl ConstructorDecl) { visit(); }
    public void visit(SingleConstructor SingleConstructor) { visit(); }
    public void visit(MultipleConstructors MultipleConstructors) { visit(); }
    public void visit(ClassFunctionsWithoutAnything ClassFunctionsWithoutAnything) { visit(); }
    public void visit(ClassFunctionsWithOnlyPar ClassFunctionsWithOnlyPar) { visit(); }
    public void visit(ClassFunctionsWithConstructor ClassFunctionsWithConstructor) { visit(); }
    public void visit(ClassFunctionsWithMethods ClassFunctionsWithMethods) { visit(); }
    public void visit(ClassFunctionsWithConstructorAndMethods ClassFunctionsWithConstructorAndMethods) { visit(); }
    public void visit(NoClassFields NoClassFields) { visit(); }
    public void visit(HasClassFields HasClassFields) { visit(); }
    public void visit(ClassBody ClassBody) { visit(); }
    public void visit(ClassExtendsError ClassExtendsError) { visit(); }
    public void visit(BaseClass BaseClass) { visit(); }
    public void visit(ExtendedClass ExtendedClass) { visit(); }
    public void visit(ClassDecl ClassDecl) { visit(); }
    public void visit(LastListVarDeclError LastListVarDeclError) { visit(); }
    public void visit(LastVarDeclaration LastVarDeclaration) { visit(); }
    public void visit(ListVarDeclError ListVarDeclError) { visit(); }
    public void visit(NonLastVarDeclaration NonLastVarDeclaration) { visit(); }
    public void visit(ListNoVarDecls ListNoVarDecls) { visit(); }
    public void visit(ListVarDeclsWithComa ListVarDeclsWithComa) { visit(); }
    public void visit(VarDecl VarDecl) { visit(); }
    public void visit(NoVarBrackets NoVarBrackets) { visit(); }
    public void visit(VarBracketsOpenClosed VarBracketsOpenClosed) { visit(); }
    public void visit(VarName VarName) { visit(); }
    public void visit(ListNoConstDeclarations ListNoConstDeclarations) { visit(); }
    public void visit(ListConstDeclarations ListConstDeclarations) { visit(); }
    public void visit(ConstBoolDecl ConstBoolDecl) { visit(); }
    public void visit(ConstCharDecl ConstCharDecl) { visit(); }
    public void visit(ConstNumberDecl ConstNumberDecl) { visit(); }
    public void visit(ConstDecl ConstDecl) { visit(); }
    public void visit(Type Type) { visit(); }
    public void visit(NoDeclarations NoDeclarations) { visit(); }
    public void visit(ClassDeclarations ClassDeclarations) { visit(); }
    public void visit(VarDeclarations VarDeclarations) { visit(); }
    public void visit(ConstDeclarations ConstDeclarations) { visit(); }
    public void visit(ProgName ProgName) { visit(); }
    public void visit(Program Program) { visit(); }


    public void visit() { }
}
